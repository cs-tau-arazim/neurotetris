...
proportion = 0.7  # TODO change?
#input_size = 234
input_size = 223
l1_size = 40  # TODO change?
l2_size = 20 # TODO change?
#output_size = 4
output_size = 14
...
def start_mutation(child):
    for j in xrange(len(child[0])):
        for k in range(len(child[0][j])):
            r = random.uniform(0, 1)
            if r < begin_mut*mutation_prob:
                r = random.uniform(0, 1)
                if r > change_prob:
                    child[0][j][k] = L1_mutation_factor*random.gauss(expected_mutation,std)
                else:
...
def generate_new_gen(last_gen):
    ...
    sorted_last_gen = sorted_last_gen[:2*num_of_keep]
    new_gen.append(sorted_last_gen[0][0])
    for i in xrange(num_of_keep-1):
        the_chosen = random.choice(sorted_last_gen)
        new_gen.append(the_chosen[0])
        sorted_last_gen.remove(the_chosen)
